...
static size_t count_requested_datasets(uint32_t qmask, uint32_t cmask)
{
    ...
    {
        if (cmask & (uint32_t)FIELD_C_X)
            ++n;
        if (cmask & (uint32_t)FIELD_C_Y)
            ++n;
        if (cmask & (uint32_t)FIELD_C_Z)
            ++n;
    }
                                 ...
                                 MPI_Comm comm)
{
    if (!pulse || !req || !out)
        return 1;
    if (req->kind != FIELD_REQ_TIMESERIES_POINTS)
        return 2;

    /* Validate request consistency */
    if (field_request_validate(req) != 0)
        return 3;
    ...
    FieldAxis t_axis;
    if (field_axis_from_spec(&t_axis, &R->t) != 0)
        return 4;
    ...
    for (int ip = 0; ip < local_np; ++ip)
    {
        ...
        for (size_t it = 0; it < nt; ++it)
        {
            ...
            {
                if (cmask & (uint32_t)FIELD_C_X)
                    local_ds[k++][idx] = (float)E[0];
                if (cmask & (uint32_t)FIELD_C_Y)
                    local_ds[k++][idx] = (float)E[1];
                if (cmask & (uint32_t)FIELD_C_Z)
                    local_ds[k++][idx] = (float)E[2];
            }
            if (qmask & (uint32_t)FIELD_Q_A)
            {
                if (cmask & (uint32_t)FIELD_C_X)
                    local_ds[k++][idx] = (float)A[0];
                if (cmask & (uint32_t)FIELD_C_Y)
                    local_ds[k++][idx] = (float)A[1];
                if (cmask & (uint32_t)FIELD_C_Z)
                    local_ds[k++][idx] = (float)A[2];
            }
        }
    }
