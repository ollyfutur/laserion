                                      ...
                                      int ax_i0, int ax_nloc)
{
    ...
    /* --- OSIRIS-compatible structure (AXIS + SIMULATION + root attrs) --- */
    {
        ...
        if (g_sim >= 0)
        {
            ...
            /* NX: use the dataset dims directly */
            {
                int nx[3] = {0, 0, 0};
                for (int i = 0; i < nd; ++i)
                    nx[i] = (int)dims[i];
            ...
            /* XMIN/XMAX in the same axis order as AXIS1.. */
            {
                double xmin[3] = {g->t_min, g->ax1_min, g->ax2_min};
                double xmax[3] = {g->t_max, g->ax1_max, g->ax2_max};
