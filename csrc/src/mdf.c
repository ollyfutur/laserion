...
int MDF_build(MDF *m,
              ...
              double dt_fs)
{
    if (!m || !pulse || !species || !Z_list || nZ == 0 || !r) return 1;
    ...
    if (!m->Z_list) { MDF_destroy(m); return 2; }
    for (size_t i = 0; i < nZ; ++i) m->Z_list[i] = Z_list[i];
    m->nZ = nZ;
    ...
    if (!(tmax > tmin)) { MDF_destroy(m); return 4; }
    ...
    if (!(dt > 0.0)) {
        ...
        if (!(lambda > 0.0)) { MDF_destroy(m); return 5; }
    ...
    if (!m->t || !m->E_abs || !m->A || !m->p) { MDF_destroy(m); return 7; }
    ...
    if (!Egrid_mut) { MDF_destroy(m); return 8; }
    ...
    if (!m->w || !m->S || !m->dP || !m->P_ion_levels) { MDF_destroy(m); return 9; }

    /* dt_local: last equals previous (Python behavior) */
    double *dt_local = (double *)malloc(N * sizeof(double));
    if (!dt_local) { MDF_destroy(m); return 10; }
    for (size_t i = 0; i + 1 < N; ++i) dt_local[i] = m->t[i + 1] - m->t[i];
    dt_local[N - 1] = dt_local[N - 2];

    double *Wint = (double *)malloc(N * sizeof(double));
    if (!Wint) { free(dt_local); MDF_destroy(m); return 11; }
